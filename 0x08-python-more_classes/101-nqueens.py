#!/usr/bin/python3
"""Solves the N-qs puzzle in python programming using OOP """
import sys


def board_init(n):
    """Initialize chessboard class in python"""
    chess_board = []
...
def board_play(chess_board, row, col):
    """ Game Play.
    Args:
    chess_board (list): currentchessboard.
    row (int): previously played row.
    col (int): previously played column.
    """
...
def board_repeat(chess_board, row, qs, board_solutions):
    """Solution
    Args:
    chess_board (list): working chessboard.
    row (int): working row.
    qs (int): number of placed qs.
